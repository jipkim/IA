...
using KNITRO
function MP(testsystemT, testsystemD, busT_gep, busD_gep, κ, setT, η, σ0_pv, σ0_wind, υ0, Solver, bounds::bounds, D_ratio, ScaleObj, SD_tol, M, N, ED, γCT, ST_res, 
      WMP, CTI, gpmax_iter, gTpo_iter, gDpo_iter, gDqo_iter, iter)   
   ...
   ##########################################################################################
   @variables(m, begin      
      Obj_DER_ct[j in 1:iter]
      0 <= gDp_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= gDq_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= aD_ct[l in setL_D, t in setT, j in 1:iter]
      0 <= vD_ct[n in setN_D, t in setT, j in 1:iter]
      fDp_ct[l in setL_D, t in setT, j in 1:iter]
      fDq_ct[l in setL_D, t in setT, j in 1:iter]
      Obj_D_prim_ct[t in setT, j in 1:iter]
      Obj_D_prim_act_ct[t in setT, j in 1:iter]
      OCgenD_ct[i in setI_D, t in setT, j in 1:iter]
      Obj_C_ct[t in setT, j in 1:iter]
      fTp_ct[l in setL_T, t in setT, j in 1:iter]
      0 <= gTp_ct[i in setI_T, t in setT, j in 1:iter]
      θT_ct[n in setN_T, t in setT, j in 1:iter]
      Obj_W_prim_ct[t in setT, j in 1:iter]
      OCgenT_ct[i in setI_T, t in setT, j in 1:iter]
      ξT_ct[l in setL_T, t in setT, j in 1:iter]
      λT_ct[setn in setN_T, t in setT, j in 1:iter]
      0 <= γTlb_ct[i in setI_T, t in setT, j in 1:iter]
      0 <= γTub_ct[i in setI_T, t in setT, j in 1:iter]
      0 <= δTlb_ct[l in setL_T, t in setT, j in 1:iter]
      0 <= δTub_ct[l in setL_T, t in setT, j in 1:iter]
      Obj_W_dual_ct[t in setT, j in 1:iter]
      λD_ct[n in setN_D, t in setT, j in 1:iter]
      μD_ct[n in setN_D, t in setT, j in 1:iter]
      βD_ct[l in setL_D, t in setT, j in 1:iter]    
      0 <= γDlb_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= γDub_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= δDlb_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= δDub_ct[i in setI_D, t in setT, j in 1:iter]
      0 <= ϑDlb_ct[n in setN_D, t in setT, j in 1:iter]
      0 <= ϑDub_ct[n in setN_D, t in setT, j in 1:iter]
      Obj_D_dual_ct[t in setT, j in 1:iter]
   end)
   ############################################
    @constraint(m, Subsidy_budget_ct[j in 1:iter], 
        sum(
            s[1,t] * (DTp[busT_gep] * loadcurveT[busT_gep,t]) 
            + sum(s[n,t] * d[n,t] for n in setN_D)            
            + sum(ST_res[n,t] * DTp[n] * loadcurveT[n,t] for n in setdiff(setN_T, [busT_gep]))
            + γCT * (
                sum(CCT * H[i] * gTp_ct[i,t,j] for i in setdiff(setI_C, setI_D))                   
            ) 
        for t in setT)
        >= 
        sum(
            τe * sum(gTp_ct[i,t,j] for i in setdiff(setI_R, setI_D))            
        for t in setT)
        + τc * DCRF * sum(gpmax_iter[j][i] for i in setdiff(setI_R_hat, setI_D))
    )
   @constraint(m, Obj_DER_def_ct[j in 1:iter], Obj_DER_ct[j] == sum(
               sum((λT_ct[busT_gep,t,j] - Coperg[i][2]) * gTp_ct[i,t,j] for i in setI_DER)
               + sum((πDER[i,t] - Coperg[i][2]) * gDp_ct[i,t,j] for i in intersect(setI_DER, setI_D))
               + sum(τe * gTp_ct[i,t,j] for i in setdiff(intersect(setI_DER, setI_R), setI_D))                  
               - sum(CCT * H[i] * gTp_ct[i,t,j] for i in setdiff(intersect(setI_DER, setI_C), setI_D))                  
         for t in setT)
         + sum(τc * DCRF * gpmax_iter[j][i] for i in intersect(setI_DER_hat, setI_R))
         - sum(Cinvg[i] * gpmax_iter[j][i] for i in setI_DER_hat)
      )
   
   @constraint(m, O_Dprim_ct[t in setT, j in 1:iter], Obj_D_prim_ct[t,j] == sum(- πDER[i,t] * gDp_ct[i,t,j] for i in setI_D))
   @constraint(m, O_Dprim_act_def_ct[t in setT, j in 1:iter], Obj_D_prim_act_ct[t,j] == 
       + sum(πC[n,t] * d[n,t] for n in setN_D)
       - sum(πDER[i,t] * gDp_ct[i,t,j] for i in setI_D)
       - λT_ct[busT_gep,t,j] * (fDp0[t])       
       )
   @constraint(m, GenCostD_ct[i in setI_D, t in setT, j in 1:iter], OCgenD_ct[i,t,j] == πDER[i,t] * gDp_ct[i,t,j])

   @constraint(m, BetweenNodes_ct[l in setL_D, t in setT, j in 1:iter], 
      1e6(
         vD_ct[linesD[l].fbus,t,j] 
         - 2*(linesD[l].r*fDp_ct[l,t,j] + linesD[l].x*fDq_ct[l,t,j])
      )
      == 1e6vD_ct[linesD[l].tbus,t,j])
   @constraint(m, RootV_ct[t in setT, j in 1:iter], vD_ct[1,t,j] == 1)

   @constraint(m, PBalance_root_ct[n in [1], t in setT, j in 1:iter], 
      sum(fDp_ct[l,t,j] for l in busesD[n].outline)            
      - (fDp0[t] - sum(gTp_ct[i,t,j] for i in setI_D))     
      - sum(gTp_ct[i,t,j] + gDp_ct[i,t,j] for i in B_gnD[n])
      + d[n,t] == 0)

   @constraint(m, PBalance_ct[n in setdiff(setN_D,[1]), t in setT, j in 1:iter], 
      sum(fDp_ct[l,t,j] for l in busesD[n].outline)  
      - sum(fDp_ct[l,t,j] for l in busesD[n].inline) 
      - sum(gTp_ct[i,t,j] + gDp_ct[i,t,j] for i in B_gnD[n])
      + d[n,t] == 0)

   @constraint(m, QBalance_ct[n in setN_D, t in setT, j in 1:iter], 
      sum(fDq_ct[l,t,j] for l in busesD[n].outline)  
      - sum(fDq_ct[l,t,j] for l in busesD[n].inline) 
      - sum(gDq_ct[i,t,j] for i in B_gnD[n]) 
      + Γpf[n] * d[n,t] == 0)



   @constraint(m, PGmax_ct[i in setI_D, t in setT, j in 1:iter], gDp_ct[i,t,j] <= gDpo_iter[j][i,t])
   @constraint(m, QGmax_ct[i in setI_D, t in setT, j in 1:iter], gDq_ct[i,t,j] <= gDqo_iter[j][i,t])
   @constraint(m, Vmax_ct[n in setN_D, t in setT, j in 1:iter], vD_ct[n,t,j] <= busesD[n].Vmax^2)
   @constraint(m, Vmin_ct[n in setN_D, t in setT, j in 1:iter], vD_ct[n,t,j] >= busesD[n].Vmin^2)
   @constraint(m, Obj_D_dual_def_ct[t in setT, j in 1:iter], Obj_D_dual_ct[t,j] ==
         sum(- (d[n,t] - (fDp0[t] - sum(gTp_ct[i,t,j] for i in setI_D)))* λD_ct[n,t,j] - Γpf[n] * d[n,t] * μD_ct[n,t,j] for n in [1])         
         + sum(- d[n,t] * λD_ct[n,t,j] - Γpf[n] * d[n,t] * μD_ct[n,t,j] for n in setdiff(setN_D,[1]))
         + sum(- busesD[n].Vmin^2 * ϑDlb_ct[n,t,j] + busesD[n].Vmax^2 * ϑDub_ct[n,t,j] for n in setdiff(setN_D,1))
         + sum(- ϑDlb_ct[n,t,j] + ϑDub_ct[n,t,j] for n in [1])         
         + sum(gDpo_iter[j][i,t] * γDub_ct[i,t,j] + gDqo_iter[j][i,t] * δDub_ct[i,t,j] for i in setI_D)
         - sum(- gTp_ct[i,t,j] * λD_ct[B_gD[i],t,j] for i in setI_D)       
      )
   
   
   @constraint(m, L2dual_gDp_ct[i in setI_D, t in setT, j in 1:iter], - πDER[i,t] + λD_ct[B_gD[i],t,j] + γDlb_ct[i,t,j] - γDub_ct[i,t,j] == 0)
   @constraint(m, L2dual_gDq_ct[i in setI_D, t in setT, j in 1:iter], + μD_ct[B_gD[i],t,j] + δDlb_ct[i,t,j] - δDub_ct[i,t,j] == 0)

   @constraint(m, L2dual_vD_ct[n in setN_D, t in setT, j in 1:iter], sum(βD_ct[l,t,j] for l in busesD[n].outline) - sum(βD_ct[l,t,j] for l in busesD[n].inline)
      + ϑDlb_ct[n,t,j] - ϑDub_ct[n,t,j] == 0)
   @constraint(m, L2dual_fDp_ct[l in setL_D, t in setT, j in 1:iter], 
      1e6(
      - sum(λD_ct[n,t,j] for n in linesD[l].fbus) + sum(λD_ct[n,t,j] for n in linesD[l].tbus)
      - 2*linesD[l].r * βD_ct[l,t,j]
      ) == 0)
   @constraint(m, L2dual_fDq_ct[l in setL_D, t in setT, j in 1:iter], 
      1e6(
      - sum(μD_ct[n,t,j] for n in linesD[l].fbus) + sum(μD_ct[n,t,j] for n in linesD[l].tbus)
      - 2*linesD[l].x * βD_ct[l,t,j]
      )
      == 0)
   @constraint(m, Utility_SD_split_ct[t in setT, j in 1:iter], Obj_D_dual_ct[t,j] - Obj_D_prim_ct[t,j] <= SD_tol)      
   @constraint(m, Oprim_ct[t in setT, j in 1:iter], Obj_W_prim_ct[t,j] == 
      - sum( (Coperg[i][2] - τe) * gTp_ct[i,t,j] for i in intersect(setI_T, setI_R))
      - sum( (Coperg[i][2] + CCT * H[i] ) * gTp_ct[i,t,j] for i in intersect(setI_T, setI_C))    
      )
   @constraint(m, DCPF_ct[l in setL_T, t in setT, j in 1:iter], fTp_ct[l,t,j] == (1/linesT[l].x) * (θT_ct[linesT[l].fbus,t,j] - θT_ct[linesT[l].tbus,t,j]))
   @constraint(m, slack_ct[t in setT, j in 1:iter], θT_ct[1,t,j] == 0)
   @constraint(m, NodeBalance_inter_ct[n in [busT_gep], t in setT, j in 1:iter],
      sum(gTp_ct[i,t,j] for i in B_gnT[n]) + sum(fTp_ct[l,t,j] for l in busesT[n].inline) - sum(fTp_ct[l,t,j] for l in busesT[n].outline)
      == DTp[n] * loadcurveT[n,t] + fDp0[t] - sum(gTp_ct[i,t,j] for i in setI_D))
   @constraint(m, NodeBalance_ct[n in setdiff(setN_T, busT_gep), t in setT, j in 1:iter],
      sum(gTp_ct[i,t,j] for i in B_gnT[n]) + sum(fTp_ct[l,t,j] for l in busesT[n].inline) - sum(fTp_ct[l,t,j] for l in busesT[n].outline)
      == DTp[n] * loadcurveT[n,t])   
   @constraint(m, gpLB_ct[i in setI_T, t in setT, j in 1:iter], gTp_ct[i,t,j] >= 0)
   @constraint(m, gpUB_ct[i in setI_T, t in setT, j in 1:iter], gTp_ct[i,t,j] <= gTpo_iter[j][i,t])
   @constraint(m, LineCapLB_ct[l in setL_T, t in setT, j in 1:iter], fTp_ct[l,t,j] >= - linesT[l].u)
   @constraint(m, LineCapUB_ct[l in setL_T, t in setT, j in 1:iter], fTp_ct[l,t,j] <= + linesT[l].u)
   @constraint(m, Odual_ct[t in setT, j in 1:iter], Obj_W_dual_ct[t,j] ==
      sum(gTpo_iter[j][i,t]*γTub_ct[i,t,j] - 0 * γTlb_ct[i,t,j] for i in setI_T)
      - sum(DTp[n] * loadcurveT[n,t] * λT_ct[n,t,j] for n in setdiff(setN_T, busT_gep))
      - (DTp[busT_gep] * loadcurveT[busT_gep,t] + fDp0[t] )* λT_ct[busT_gep,t,j]   
      + sum(linesT[l].u * δTub_ct[l,t,j] + linesT[l].u * δTlb_ct[l,t,j] for l in setL_T)
      )
   @constraint(m, const_gTp_R_ct[i in setdiff(intersect(setI_T, setI_R), setI_DER), t in setT, j in 1:iter], - γTlb_ct[i,t,j] + γTub_ct[i,t,j] - λT_ct[B_g[i],t,j] == - (Coperg[i][2] - τe))
   @constraint(m, const_gTp_C_ct[i in setdiff(intersect(setI_T, setI_C), setI_DER), t in setT, j in 1:iter], - γTlb_ct[i,t,j] + γTub_ct[i,t,j] - λT_ct[B_g[i],t,j] == - (Coperg[i][2] + CCT * H[i] ))
   @constraint(m, const_gTp_DER_R_ct[i in intersect(setI_DER, setI_R), t in setT, j in 1:iter], - γTlb_ct[i,t,j] + γTub_ct[i,t,j] - λT_ct[busT_gep,t,j] == - (Coperg[i][2] - τe))
   @constraint(m, const_gTp_DER_C_ct[i in intersect(setI_DER, setI_C), t in setT, j in 1:iter], - γTlb_ct[i,t,j] + γTub_ct[i,t,j] - λT_ct[busT_gep,t,j] == - (Coperg[i][2] + CCT * H[i] ))
   @constraint(m, const_fTp_ct[l in setL_T, t in setT, j in 1:iter], - δTlb_ct[l,t,j] + δTub_ct[l,t,j] + ξT_ct[l,t,j] - λT_ct[linesT[l].tbus,t,j] + λT_ct[linesT[l].fbus,t,j] == 0)
   @constraint(m, const_θT_ct[n in setN_T, t in setT, j in 1:iter], - sum(ξT_ct[l,t,j]/linesT[l].x for l in busesT[n].outline) + sum(ξT_ct[l,t,j]/linesT[l].x for l in busesT[n].inline) == 0)
   @constraint(m, WM_SD_split_ct[t in setT, j in 1:iter], Obj_W_dual_ct[t,j] - Obj_W_prim_ct[t,j] <= SD_tol)   
   @constraint(m, GenCostT_ct[i in setI_T, t in setT, j in 1:iter], OCgenT_ct[i,t,j] == Coperg[i][2]*gTp_ct[i,t,j])
   @objective(m, Max, Obj_REG)
   @constraint(m, Obj_DER_cut[j in 1:iter], Obj_DER >= Obj_DER_ct[j])   
   optimize!(m)
   return m
end
