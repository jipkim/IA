...
function NetworkLoad(testsystem)
   if isfile(string(pwd(),"/data/","$testsystem.mat"))
      usingMatPower = 1
      PUcorrection = 100
   else      
      usingMatPower = 0
   ...
   #    usingMatPower = 0
   # elseif testsystem == "NYISO" || testsystem == "NYISO_v2" || testsystem == "NYISO_v2_ext" || testsystem == "Manhattan" || testsystem == "Manhattan2"
   #       usingMatPower = 0
   # else
   #    usingMatPower = 1
   #    PUcorrection = 100
...
#    genset = 1:length(generators)
   ...
   return buses, lines, generators, datamat, lineset, busset, Dp, Dq
end
